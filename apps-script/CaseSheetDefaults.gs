...
function case_defaultConnections_() {
  return [
    ...
    ["IMPLANT", "N", "구강악안면외과", "임플란트 현황", "", "임플 현황", "A", "B", 3, "", 90, "CONFIG", "", "검토필요", "현재 학년·A/O 의미 확인 전 비활성"],
...
function case_defaultMappings_() {
  ...
  return [
    ...
    case_mapping_("OM_PT", "검토필요", "OM", "Physical therapy 완료+예정", "VALUE(I)", "VALUE(J)", "SUM(I,J)", case_target_(om, "증례별 임상참여", "Physical therapy"), "환자수", "SUM", 80, "I/J가 8점 단위인지 환자 건수인지 확인 전 비교 보류; 예정 포함"),
    ...
    case_mapping_("IMPLANT_TOTAL", "검토필요", "IMPLANT", "Implant 3-2", "VALUE(C)", "", "VALUE(C)", [case_target_(oms, "증례별 임상참여", "Implant 1st op. (2개 이하 식립)_[O]"), case_target_(oms, "증례별 임상참여", "Implant 1st op (3개 이상 식립)_[O]")].join("\n"), "승인수", "SUM", 90, "임플 현황 탭의 학년·A/O·식립 개수 의미 확인 전 비교 보류"),
    ...
    case_mapping_("OMS_RECALL_MINOR", "승인", "OMS", "Minor recall", "VALUE(R)", "", "VALUE(R)", case_target_(oms, "증례별 임상참여", "외래 Recall check minor"), "승인수", "SUM", 70, "승인 건수 비교"),
    case_mapping_("OMS_NEW_CHART", "승인", "OMS", "신환 차팅", "VALUE(S)", "", "VALUE(S)", case_target_(oms, "증례별 임상참여", "외래 신환 차팅"), "승인수", "SUM", 70, "승인 건수 비교"),
    ...
    (function () {
      var row = case_mapping_("PROS_LAB", "검토필요", "PROS", "기공", "VALUE(X)", "", "VALUE(X)", [case_target_(pros, "기공", "[가철성]Laboratory case evaluation"), case_target_(pros, "기공", "[고정성]Laboratory case evaluation"), case_target_(pros, "기공", "[임플란트]Laboratory case evaluation")].join("\n"), "승인수", "SUM", 60, "X/Y가 어느 기공 항목인지 확인 전 비활성");
      row[1] = "N";
      return row;
    })(),
